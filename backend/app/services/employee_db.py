...
def list_employees(session:Session,search:str='')->dict[str,Any]:
 q=select(Employee,EmploymentRecord).join(EmploymentRecord,EmploymentRecord.employee_id==Employee.id).where(Employee.deleted_at.is_(None),EmploymentRecord.status=='active')
 if search:q=q.where(or_(Employee.name.ilike(f'%{search}%'),Employee.phone.ilike(f'%{search}%')))
 rows=session.execute(q.order_by(Employee.id.desc())).all();return {'rows':[serialize_employee(e,w,session) for e,w in rows],'total':len(rows)}
...
def unsigned_contract_warnings(session:Session,today:date|None=None)->list[dict[str,Any]]:
 ...
 return out
def list_contracts(session:Session,eid:int|None=None):
 q=select(Contract).where(Contract.status!='terminated')
 if eid:q=q.where(Contract.employee_id==eid)
 return [{'id':x.id,'employee_id':x.employee_id,'contract_type':x.contract_type,'contract_no':x.contract_no,'sign_date':x.sign_date.isoformat() if x.sign_date else None,'start_date':x.start_date.isoformat(),'end_date':x.end_date.isoformat(),'status':x.status,'remark':x.remark} for x in session.scalars(q.order_by(Contract.id.desc())).all()]
...
def employee_detail(session:Session,eid:int)->dict[str,Any]|None:
 ...
 return {'employee':serialize_employee(e,w,session),'contracts':list_contracts(session,eid),'attendance':[dict(x) for x in attendance],'payroll':[dict(x) for x in payroll]}
