...
@router.get('')
def ls(search:str='',db:Session=Depends(get_db)):return list_employees(db,search)
@router.post('')
...
def contracts(employee_id:int|None=None,db:Session=Depends(get_db)):return {'rows':list_contracts(db,employee_id)}
...
def employees_export(db:Session=Depends(get_db)):
    ...
    ws.append(["姓名","身份证(脱敏)","手机号","性别","企业","岗位","入职日期","状态"])
    for r in data["rows"]:
        ws.append([r["name"],r["id_card_masked"],r["phone"],"男" if r["gender"]=="male" else "女","",r["company_name"],r["position_name"],r["entry_date"],"在职" if r["status"]=="active" else "离职"])
    stream=BytesIO();wb.save(stream);stream.seek(0)
    return StreamingResponse(stream,media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",headers={"Content-Disposition":'attachment; filename="employees.xlsx"'})

@router.get('/contracts/export/file.xlsx')
def contracts_export(db:Session=Depends(get_db)):
    from io import BytesIO; from openpyxl import Workbook; from fastapi.responses import StreamingResponse
    rows=list_contracts(db); wb=Workbook(); ws=wb.active; ws.title="合同"
    ...
    stream=BytesIO(); wb.save(stream); stream.seek(0)
    return StreamingResponse(stream,media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",headers={"Content-Disposition":'attachment; filename="contracts.xlsx"'})
